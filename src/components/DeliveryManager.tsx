...
export const DeliveryManager = ({ order, onOrderUpdate }: DeliveryManagerProps) => {
  ...
  const handleRequestDelivery = async () => {
    setIsLoading(true);
    try {
      const updatedOrder = await orderService.requestIFoodDelivery(order.id);
      onOrderUpdate(updatedOrder);
      
      toast({
        title: "Entrega solicitada! 🚚",
        description: `Pedido #${order.id} enviado para entrega via iFood`,
      });
    } catch (error) {
  ...
  const handleRefreshStatus = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Status da entrega atualizado para ${getDeliveryStatusLabel(updatedOrder.delivery?.status)}`,
      });
    } catch (error) {
  ...
  const handleCancelDelivery = async () => {
    setIsLoading(true);
    try {
      const updatedOrder = await orderService.cancelDelivery(order.id, 'Cancelado pelo restaurante');
      onOrderUpdate(updatedOrder);
      
      toast({
        title: "Entrega cancelada! ❌",
        description: `Entrega do pedido #${order.id} foi cancelada`,
      });
    } catch (error) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure a entrega para este pedido
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Este pedido ainda não possui entrega configurada.
            </p>
            <Button 
            ...
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Tem certeza que deseja cancelar a entrega do pedido #{order.id}?
